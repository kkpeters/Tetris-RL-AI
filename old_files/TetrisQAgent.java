    ...
    extends QAgent
{
    ...
    public Mino getExplorationMove(final GameView game) {
        ...
        List<Mino> allMinoMoves = game.getFinalMinoPositions();
        // Matrix qFunctions = Matrix.zeros(1, numMinoPositions);
        double[] qFunctions = new double[allMinoMoves.size()];

        for(int i = 0; i < allMinoMoves.size(); i++){
        ...
        // highest q value
        double maxQValueIndex = -1;
        double maxQValue = Double.NEGATIVE_INFINITY;
        for (int i = 0; i < qFunctions.length; i++) {
            if (qFunctions[i] > maxQValue) {
                maxQValue = qFunctions[i];
                maxQValueIndex = i;
            }
        }
        ...
        */
        ...
        // calculating the probability of getting the best move
        double moveProbability[] = new double[allMinoMoves.size()];
        double probabilitySum = 0.0;
        for (int i = 0; i < moveProbability.length; i++) {
            if (i == maxQValueIndex) {
                moveProbability[i] = Math.exp(-qFunctions[i]);
                probabilitySum += Math.exp(-qFunctions[i]);
            }
            else {
                moveProbability[i] = 0.1 / (moveProbability.length -1);
            }
        }
        ...
        // for (int i = 0; i < numMinoPositions; i++) {
        for (int i = 0; i < moveProbability.length; i++) {
            moveProbability[i] /= probabilitySum;
        }
        /*
        for (int i = 0; i < numMinoPositions; i++) {
            if (outcome[i] < minVal) {
        ...
        */
        double randomMove = this.getRandom().nextDouble();
        double totalProbability = 0.0;
        for (int i = 0; i < moveProbability.length; i++) {
            totalProbability += moveProbability[i];
            if (randomMove <= totalProbability) {
                return allMinoMoves.get(i);
            }
        }

        //return game.getFinalMinoPositions().get(minValPos);
        return allMinoMoves.get(moveProbability.length -1);
